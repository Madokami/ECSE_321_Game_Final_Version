...
import game.Game;
import graphicEffects.SpecialEffects;
...
public class MenuChar implements GeneralMenu{
		...
		public Game game;
		public static AttributeHandler handler;
		...
		public MenuChar(Game game){
			...
			levelImage = loader.loadImage("/image/portraits/level.png");
			...
			statsX=GameSystem.ABSWIDTH/2+this.statsSelectionCombinedOff.getHeight()/5;
			statsY=GameSystem.ABSHEIGHT/2-this.statsSelectionCombinedOff.getHeight()/2;
		...
		public void render(Graphics g){
			if(isChooseChar()){
				...
				g.drawImage(levelImage, statsX+50, statsY-130, null);
				for(int i=0;i<handler.levelImg.length;i++){
					g.drawImage(handler.levelImg[i], statsX+150+i*20, statsY-130,24,32, null);
				}
		...
		public synchronized void renderSelected(Graphics g){
			if(isChooseChar()){
				...
				g.drawString("Choose Character", Menu.X_START+Menu.SPACING/2, Menu.Y_START+50);
			}
			else if(isDisplayStatus()){
		...
		public synchronized void transferPlayerData(){
			...
			GameSystem.sendCommandToOther("sendBP");
			GameSystem.sendCommandToOther("sendBombs");
			GameSystem.sendCommandToOther("sendBombL");
			GameSystem.sendCommandToOther("sendSpd");
			GameSystem.sendCommandToOther("sendSoul");
		...
		public synchronized void setSecondPlayer(){
			if(cSelected == CHARACTER.MADOKA){
				...
				GameSystem.sendCommandToOther("setHo");
			}
			else if(cSelected == CHARACTER.SAYAKA){
				GameSystem.sendCommandToOther("setSa");
			}
			else if(cSelected == CHARACTER.MAMI){
				GameSystem.sendCommandToOther("setMa");
			}
			else if(cSelected == CHARACTER.KYOUKO){
				GameSystem.sendCommandToOther("setKy");
			}
		}
